...
export function buildContextBlock(citations: Citation[]): string {
  ...
  return (
    `\n\n--- 【参考資料】 ---\n` +
    `以下の公的資料が与えられています。回答では必ず、関連する資料の名称を文中で明示し（例：「〇〇によれば」「△△では…と示されています」）、該当箇所を引用または具体的に要約しながら、資料に基づいて答えてください。` +
    `一般論で済ませず、資料に書かれている事実・基準・考え方を根拠として具体的に示すこと。ただし資料にない事柄を推測で断定しないこと。\n${blocks}\n--- 参考資料ここまで ---`
  );
}
